...
/*
 * Constructor for both live and offline capture modes
 */
...
void HttpSniffer::start()
{
  ...
  if (m_offline) {
     /* open pcap capture file (debugging/testing mode) */
     handle = pcap_open_offline(m_pcap_filename.c_str(), errbuf);
     if (handle == NULL) {
        throw runtime_error(str(boost::format("Couldn't open file %s") % errbuf));
     }
  }
  else {
